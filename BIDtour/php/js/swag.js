var SwagRouter = Backbone.Router.extend({
	oContinentList : new Backbone.Collection(),
	...
	initialize : function() {
		this.oContinentList.url = 'continent/';
		this.oContinentList.fetch({
	...
	renderNav: function(){
		this.oContinentList.models.forEach(function(oContinent) {
			// we append a single cd into the cdlist div
			jQuery("#navPlaceHolder").append(this.fNavTemplate(oContinent.attributes));
		}, this);
	...
	renderItem: function(){
		...
		this.oImageList.url = 'continent/' + this.nContinentId + '/places/' + this.nProductId + '/images/';
	...
	showHomepage : function(nContinentId, nProductId) {
		jQuery("#product").show();
		jQuery("#contact").hide();
		jQuery("#about").hide();
		jQuery("#home").hide();
		
		if(nContinentId == null){
			nContinentId = this.oContinentList.at(0).id;
		}else if(isNaN(parseInt(nContinentId))){
			nContinentId = this.oContinentList.findWhere({name: nContinentId}).id;
		}
		this.nProductId = nProductId;
		if(this.nContinentId != nContinentId ){
			this.nContinentId = nContinentId;
			this.oItemList.url = 'continent/homepage/';
	...
	showProduct : function(nContinentId, nProductId) {
		jQuery("#product").show();
		jQuery("#contact").hide();
		jQuery("#about").hide();
		jQuery("#home").hide();
		...
		if(this.nContinentId != nContinentId ){
			this.nContinentId = nContinentId;
			this.oItemList.url = 'continent/' + nContinentId + '/places/';
			this.oItemList.fetch({
				success : jQuery.proxy(this.renderItems, this),
				error : function(collection, err) {
					throw err.status + " " + err.statusText;
